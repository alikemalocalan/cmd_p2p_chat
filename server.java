...
class voiceserver extends JPanel{
    ...
    public static void main(String args[]) throws Exception {
        try {
            soket = new ServerSocket(43333).accept();
        }
        catch (Exception e1) {
            e1.printStackTrace();
            System.out.println("baglanti koptu");
        }
        soketses= new ServerSocket(42222).accept();
        JFrame frame= new JFrame("Sifreli chat-Server");
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setContentPane(new voiceserver());
...
class SoundReceiver implements Runnable{
    ...
    public SoundReceiver() throws IOException, LineUnavailableException {
        inSpeaker = (SourceDataLine) AudioSystem.getLine(new DataLine.Info(SourceDataLine.class,voiceserver.af));
        inSpeaker.open(voiceserver.af);
    }
    public void run(){
        int bytesRead= 0;
        ...
        while(bytesRead!= -1){
            try {
                bytesRead = new DataInputStream(voiceserver.soketses.getInputStream()).read(insound,0,insound.length);
            } catch (IOException e) {
                e.printStackTrace();
            }
            if (bytesRead>= 0){
